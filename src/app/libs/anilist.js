...
export async function fetchMangaCharacters(title) {
  const query = `
    query ($search: String) {
...
export async function fetchNewsAnime() {
  const query = `
    query {
      Page(perPage: 10) {
...
export async function fetchTrendingAnimePaginated(page = 1, perPage = 20) {
  const query = `
...
export async function fetchSimilarAnime(id) {
  const query = `
    query ($id: Int) {
      Media(id: $id, type: ANIME) {
...
export async function fetchAnimeByGenre(genre, page = 1, isAdult = false) {
  const query = `

...
export default function ReadPage() {
  ...
  // ✅ PARAMS SESUAI URL /manga/read/[mangaSlug]/[chapterId]
  ...
  useEffect(() => {
    ...
    const loadImages = async () => {
      try {
        ...
        const files =
          chapter.data?.length > 0 ? chapter.data : chapter.dataSaver

        const modeStr =
          chapter.data?.length > 0 ? 'data' : 'data-saver'

        const urls = files.map(
          (file) =>
            `${chapter.baseUrl}/${modeStr}/${chapter.hash}/${file}`
        )
  ...
  // ✅ NAVIGASI HARUS BAWA mangaSlug
  const handleNav = (id) => {
    if (!id) return
    router.push(`/manga/read/${mangaSlug}/${id}`)
    setCurrentPage(0)

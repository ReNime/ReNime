...
export default function MangaLandingPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Fetch user session and popular manga in parallel
        const [userSession, popularRes] = await Promise.all([
          AuthUserSession(),
          fetchPopularManga()
        ])
  ...
  return (
    <>
      <Head>
        <title>Manga | ReNime</title>
        <meta
          name="description"
          content="Discover the hottest manga with a full-featured reader."
        />
        <meta property="og:title" content="Manga | ReNime" />
        <meta

...
export default function Chat() {
  ...
  useEffect(() => {
    const savedMessages = localStorage.getItem("renai-chat-history");
    if (savedMessages) {
      try {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      localStorage.setItem("renai-chat-history", JSON.stringify(messages));
    }
  }, [messages]);
  ...
  const clearChat = () => {
    if (confirm("Clear all chat history?")) {
      setMessages([]);
      localStorage.removeItem("renai-chat-history");
    }
  };
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-sm sm:text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center gap-1.5 sm:gap-2 truncate">
                <span className="truncate">Renai AI</span>
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">
                <FaCircle
                  size={5}
                  className="text-emerald-500 animate-pulse flex-shrink-0"
                />
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="hidden md:inline">
                {personaConfig[persona].name.split(" ")[0]}
              </span>
              <FaChevronDown
                size={10}
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center px-3 sm:px-4">
            ...
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-800 dark:text-slate-200 mb-2 sm:mb-3 flex items-center justify-center gap-2">
              Konnichiwa! I'm Renai! <FaHeart className="text-pink-500" />
            </h2>

            <p className="text-sm sm:text-base text-slate-600 dark:text-slate-400 max-w-md mb-4 sm:mb-6">
              Your anime-loving AI assistant powered by multiple AI providers.
              Ask me anything about anime, manga, or just chat!
            </p>

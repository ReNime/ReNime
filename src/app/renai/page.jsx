  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      type: "text",
      content: "Hi I'm **Renai**, your AI assistant for anime, manga, manhwa, manhua, and light novels. You can chat or upload a screenshot via menu to identify an anime instantly!",
    },
  ]);
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Renai is your AI assistant for anime, manga, manhwa, and light novels." />
      </Head>
      ...
      <main className="flex flex-col items-center min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 text-white relative overflow-hidden">
        ...
        <div className="w-full max-w-6xl flex flex-col h-screen px-3 sm:px-6 lg:px-8 relative z-10">
          ...
          <section className="flex-1 overflow-y-auto py-6 space-y-5 px-1">
            {messages.map((msg, i) => (
              <motion.div
                key={i}
              ...
              >
                {msg.type === "text" && (
                  <div className={`px-5 py-4 rounded-3xl max-w-[85%] text-sm sm:text-base shadow-xl ${
                    msg.role === "user"
                      ? "bg-gradient-to-br from-blue-500 via-blue-600 to-cyan-500 text-white"
                      : "bg-slate-800/60 border border-blue-500/20 text-slate-100"
                  }`}>
          ...
          <footer className="p-3 sm:p-4 bg-slate-900/40 backdrop-blur-2xl rounded-t-2xl border-t border-blue-500/20 mb-2 sm:mb-4">
            {!isLoggedIn ? (
              <button
                onClick={() => signIn()}
